...
- **Cursor 常時ルール**: `.cursor/rules/plan-supremacy.mdc`（**§5 外の無断作成・削除禁止**・「実装して」単体では許可にならない等）。エージェントはこれに従う。
- **Plan モード**: ユーザーに提示する **PLAN 案の本文に、未承認の項目を含めない**（実装前だけでなく、**案に書き込むこと自体**要承認。追加したいことは PLAN 外で列挙し Yes 後にのみ PLAN に統合）。
- **作成・削除**: 対象パスを列挙してユーザーに示し、**Yes（許可）があるまで実行しない**（ツール含む）。**§5 に無い新規パス**が必要なときは、**追記案（§4.3・§5 ツリー）を示してから** Yes 待ち。無断で作らない。
- **新規ファイル**: **[§5](#sec-5) に無いパス**は**原則作らない**。**既存ファイルへの追記・集約**で対応する（例: `03-analysis/maplibre` の `index.html` / `main.js`）。
