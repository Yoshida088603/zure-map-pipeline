...
# GPKG レイヤに対する SpatiaLite ST_MakeValid 用 SELECT（stdin なし、引数: gpkg パス レイヤ名）
zure_sql_st_makevalid() {
  python3 - "$1" "$2" <<'PY'
...
gpkg, layer = sys.argv[1], sys.argv[2]
raw = subprocess.check_output(["ogrinfo", "-json", gpkg, layer], text=True)
j = json.loads(raw)
ly = j.get("layers", [{}])[0]
fields = [f["name"] for f in ly.get("fields", [])]
geom = "geom"
parts = [f"ST_MakeValid({q(geom)}) AS {q(geom)}"]
for fn in fields:
    parts.append(q(fn))
select_list = ", ".join(parts)
print(f"SELECT {select_list} FROM {q(layer)}")
PY
}
...
run_zure() {
  ...
  (
    ...
    if [[ -n "${ZURE_ONLY_KEI:-}" ]]; then
      local _want _rawk _k _new=()
      IFS=',' read -ra _rawk <<< "$ZURE_ONLY_KEI"
      for _k in "${_rawk[@]}"; do
        _k="${_k// /}"
        [[ -z "$_k" ]] && continue
        if [[ "$_k" =~ ^[0-9]+$ ]]; then
    ...
    if [[ ${#KEI_LIST[@]} -eq 0 ]]; then
      echo "Error: 処理対象の系がありません（ZURE_ONLY_KEI または RAW 配置を確認）" >&2
      exit 1
    fi
    for k in "${KEI_LIST[@]}"; do
      local n=$((10#$k))
      ...
      if [[ "${ZURE_TWO_PASS:-1}" == "1" ]]; then
        local step1="$DIR_MERGE_BEFORE/${k}.step1.tmp.gpkg"
        rm -f "$step1" "$out_gpkg"
        ogr2ogr -t_srs "$T_SRS" -f GPKG -nln "$LAYER_NAME" "$step1" "$vrt" 2>&1
        rm -f "$vrt"
        local sql
        sql=$(zure_sql_st_makevalid "$step1" "$LAYER_NAME")
        ogr2ogr -f GPKG -nln "$LAYER_NAME" "$out_gpkg" "$step1" -dialect sqlite -sql "$sql" 2>&1
        rm -f "$step1"
      else
    ...
    for k in "${KEI_LIST[@]}"; do
      while IFS= read -r -d '' fsum; do
        ...
        exp_zu=$((exp_zu + fc))
      done < <(zure_find_shp_print0 "$ORIGIN_ROOT/$k")
    done
    act_zu=0
    for k in "${KEI_LIST[@]}"; do
