...
var OVERVIEW_DETAIL_CUTOFF_Z = 8;
...
if (isZ12KeiMode || isAllKeiPmtilesMode || isOverviewMode) {
...
var map = new maplibregl.Map({
  ...
  center: isOverviewMode
    ? [137.5, 36.2]
    : useMultiKeiZureDetail
      ? [137.5, 36.2]
      : isZ12KeiMode || isAllKeiPmtilesMode
        ? [139.45, 35.68]
        : [139.619109, 35.768183],
  zoom: isOverviewMode ? 5.2 : useMultiKeiZureDetail ? 5.2 : isZ12KeiMode ? 6 : 12,
  minZoom: 0,
  maxZoom: 22,
});
...
/** overview.pmtiles・市区町村ポリゴン。maxZoom を渡すとズーム以上で非表示（系別タイルと切替） */
function addPmtilesOverviewMunicipalityFill(map, maxZoom) {
  var ovDir = PMTILES_ZUREMAP_DIR.replace(/^\/+|\/+$/g, '');
  var ovUrl = DATA + '/' + ovDir + '/overview.pmtiles';
  if (!map.getSource('pmtiles_overview')) {
    map.addSource('pmtiles_overview', {
      type: 'vector',
      url: 'pmtiles://' + ovUrl,
    });
  }
  if (map.getLayer('overview_municipality_fill')) return;
  var spec = {
    id: 'overview_municipality_fill',
    type: 'fill',
    source: 'pmtiles_overview',
    'source-layer': 'overview_municipality',
    filter: zurePolygonGeomFilter,
    paint: {
      'fill-color': [
        'case',
        ['==', ['to-number', ['get', 'has_data']], 1],
        '#2ecc71',
        '#bdbdbd',
      ],
      'fill-opacity': 0.52,
      'fill-outline-color': 'rgba(0,0,0,0.18)',
    },
  };
  if (maxZoom != null && !Number.isNaN(Number(maxZoom))) spec.maxzoom = Number(maxZoom);
  map.addLayer(spec);
  map.on('click', 'overview_municipality_fill', function (e) {
    var lng = e.lngLat.lng.toFixed(6);
    var lat = e.lngLat.lat.toFixed(6);
    var p = e.features[0].properties;
    showAttributePanel('市区町村 overview', lng, lat, p, { has_data: 'has_data', fid: 'fid' }, 'overview_属性');
  });
  installZureHoverCursor(['overview_municipality_fill']);
}
...
map.on('load', async () => {
  if (isOverviewMode) {
    addPmtilesOverviewMunicipalityFill(map);
    return;
  }
  ...
  if (useMultiKeiZureDetail) {
    addPmtilesOverviewMunicipalityFill(map, OVERVIEW_DETAIL_CUTOFF_Z);
    allKeiFillLayerIds = [];
    ...
    ALL_KEI_PMTILES_STEMS.forEach(function (stem) {
      ...
      map.addLayer({
        ...
        minzoom: OVERVIEW_DETAIL_CUTOFF_Z,
  ...
  if (isZ12KeiMode && !useMultiKeiZureDetail) {
    addPmtilesOverviewMunicipalityFill(map, OVERVIEW_DETAIL_CUTOFF_Z);
