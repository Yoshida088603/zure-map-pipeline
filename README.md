...
| **47** | `02-convert/47-geopackage-per-kei2pmtiles.sh` | **`geopackage_per_kei/*.gpkg` をそのまま PMTiles 化**（マスク・分割なし）。**`NN.gpkg` → `data/05-pmtiles/NN.pmtiles`**。内部で **45** を呼び出し（**既定 z0–11**・MAX 既定どおり。z12 にしたいときは `PMTILES_MAXZOOM=12`）。入力は既定で `data/03-geopackage/shp2geopackage/geopackage_per_kei`（`20 zure` の run 配下ならその `…/geopackage_per_kei` を第1引数で指定）。 |
| **46a** | `02-convert/46-split-gpkg-tokyo-other.sh` | （任意）**GPKG を東京都とそれ以外に分割**。PMTiles が重すぎるときの容量対策。 |
| **46b** | `02-convert/46-split-gpkg-fid-half.sh` | （任意）**FID 前後半で 2 分割**。 |
| **46c** | `02-convert/46-split-gpkg-nokitakanto.sh` | （任意）**指定 PREFCODE を属性で除いた 1 本の GPKG**（マスクは任意）。 |
| **50** | `02-convert/50-check-pmtiles.sh` | **GDAL の PMTiles ドライバ登録と最小 GPKG→PMTiles 書き出し**の環境スモーク。特定の本番 GPKG／PMTiles との論理突合はしない。 |
| （検図） | `03-analysis/maplibre/serve.py` | PMTiles を **Range 対応 HTTP** で配信。`python -m http.server` では足りない。8080 占有時は `python3 serve.py 8765`（下記「系別 PMTiles 検図」参照）。 |
...
| **地図** | `main.js` の系別モード（`?mode=z12`／互換 `z13`）では **`maxZoom: 22`**。z11 より先は **overzoom**（拡大はぼやけるが操作は継続）。 |
| **URL** | クエリ名 `mode=z12` は**互換のため維持**（PMTiles の実 maxzoom とは数字が一致しない）。 |
| **起動** | `cd 03-analysis/maplibre && python3 serve.py`（ポート変更: `python3 serve.py 8765`）。 |

**系別 z0–11 検図**（`serve.py` 起動後、ポートを読み替え）:

| 用途 | URL 例（既定ポート 8080） |
|------|---------------------------|
| **`05-pmtiles` の系別 PMTiles をすべて重畳** | [http://localhost:8080/03-analysis/maplibre/index.html?mode=all-kei](http://localhost:8080/03-analysis/maplibre/index.html?mode=all-kei) |
| **1 系だけ**（既定 09） | [http://localhost:8080/03-analysis/maplibre/index.html?mode=z12](http://localhost:8080/03-analysis/maplibre/index.html?mode=z12) |
| **系を指定** | [http://localhost:8080/03-analysis/maplibre/index.html?mode=z12&kei=03](http://localhost:8080/03-analysis/maplibre/index.html?mode=z12&kei=03) |
| **ファイルを明示** | `?mode=z12&pmtiles=05-pmtiles/15.pmtiles`（`15` は 2 桁 `kei` 未対応のため） |

**開かない・真っ白（Cursor 利用時）**: `serve.py` は **WSL 内**で動きます。**Cursor の Simple Browser / 組み込みブラウザ**（`[CursorBrowser]` ログが出る環境）は、WSL の `localhost` に**届かない**ことが多いです。**Windows の Chrome / Edge / Firefox** で開いてください。例（ポート 8765・全系）: [http://127.0.0.1:8765/03-analysis/maplibre/index.html?mode=all-kei](http://127.0.0.1:8765/03-analysis/maplibre/index.html?mode=all-kei)（ポートは `serve.py` の引数に合わせる）。`serve.py` 起動時に **WSL の IP** が表示されたら、Windows から `http://そのIP:ポート/...` も試せます。
