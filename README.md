...
| **40** | `02-convert/40-merge-csv-sauce-geopackage.sh` | **用途別マージ** → `data/04-merge-geopackage/`。`tochi`/`gaiku`/`toshi`/`kozu`（CSV 系）に加え、ずれまっぷ SHP 経路は **`zure`**（`20` の `geopackage_per_kei` を統合）。 |
| **42** | `02-convert/42-check-merged-geopackage.sh` | **統合 GPKG の ogrinfo 出力**（レイヤ名・件数・投影などの**目視確認用**。自動の合格／不合格判定はしない）。 |
...
bash 02-convert/40-merge-csv-sauce-geopackage.sh all
bash 02-convert/42-check-merged-geopackage.sh
...
`20` は **SHP→GPKG（系別 `geopackage_per_kei/`）まで**。全系の 1 本への統合は **`40-merge-csv-sauce-geopackage.sh zure`**。**系別のまま PMTiles 検図**するなら **`47`**、全国 1 本のマージ済み GPKG から作るなら **`45`**（入力に `data/04-merge-geopackage/公図と現況のずれデータ_merged.gpkg` 等）。
...
# マージ経路の例
bash 02-convert/40-merge-csv-sauce-geopackage.sh zure
